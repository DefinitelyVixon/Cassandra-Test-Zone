...
class QueryManager:
    ...
    def __init__(self, keyspace):
        ...
        self.queries = {
            ...
            'update_std_name': self.session.prepare(
                "UPDATE gpa_table SET student_name=? WHERE dep_code=? AND semester=? AND student_id=?;")
        }
        self.contents = None
    ...
    def insert(self, student_df=None):
        if student_df is None:
            student_df = QueryManager.read_excel()
        try:
...
if __name__ == '__main__':
    ...
    semesters = sheet_names
    selected_df = pd.DataFrame(columns=['student_id', 'gpa', 'student_name', 'semester', 'reg_year', 'dep_code'])

    for sem in semesters:
        semester_grades = qm.select('0602', sem)
        selected_df = pd.concat([selected_df, semester_grades], ignore_index=True)

    selected_df = selected_df.sort_values(by='gpa', ascending=False, ignore_index=True).dropna()
    selected_df['reg_year'] = selected_df['reg_year'].astype(np.int32)
